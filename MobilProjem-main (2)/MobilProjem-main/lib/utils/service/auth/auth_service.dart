...
class AuthService implements IAuthService {
  ...
  Future<void> rezervasyonlariOlustur() async {
    ...
    final String bugun = "${DateTime.now().year}-${DateTime.now().month}-${DateTime.now().day}";
    ...
    for (String saat in saatler) {
      ...
      final docRef = _firestore.collection(_rezervasyonlarCollection).doc(saatId);
  ...
  try {
    ...
    final rezervasyonDoc = await _firestore
